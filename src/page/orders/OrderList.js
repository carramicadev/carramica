...
const OrderList = () => {
    ...
    try {
      if (e.target.value !== "") {
        ...
        arrayField[indexOrder] = {
          ...arrayField[indexOrder],
          resi: e.target.value,
          resiCreatedBy: currentUser?.uid,
          orderStatus: "sent",
        };

        // Update the document with the modified array
        await updateDoc(getDocOrd, {
          orders: arrayField,
  ...
  const handlecreateInv = async (id) => {
    try {
      ...
      setUpdate((prevValue) => !prevValue);
      enqueueSnackbar(
        "invoice berhasil dibuat, silahkan cklick ulang invoice id untuk melihat invoice!",
        { variant: "success" }
      );
    } catch (e) {
      console.log(e.message);
  ...
  const [column, setColumn] = useState([
    ...
    { label: "Nama pengirim", key: (item) => item?.senderName, style: {} },
    {
      ...
      key: (item) =>
        item?.isInvWASent ? (
          <p style={{ color: "gray" }}>{item?.senderPhone}</p>
        ) : (
          <a
            ...
            onClick={() =>
              setSendWADialog({
                ...
                data: item,
                type: "pembayaran",
                message: "Kirim link pembayaran ke pengirim?",
              })
            }
          >
            {item?.senderPhone}
    ...
    { label: "Nama Penerima", key: (item) => item?.receiverName, style: {} },
    {
      ...
      key: (item) =>
        item?.isResiWASent ? (
          <p style={{ color: "gray" }}>{item?.receiverPhone}</p>
        ) : item?.resi ? (
          <a
            style={{ textDecoration: "underline" }}
            href="#"
            onClick={() =>
              setSendWADialog({
                open: true,
                data: item,
                type: "resi_to_receiver",
                message: "Kirim resi ke pemerima?",
              })
            }
          >
            {item?.receiverPhone}
    ...
    },
    {
      ...
      key: (item) =>
        typeof item?.createdAt === "number" ? (
          <TimestampToDate timestamp={item?.createdAt} />
        ) : (
    ...
    },
    {
      ...
      key: (item, i, idOrder, style) => (
        <li style={style}>{item?.paymentStatus || "Unpaid"}</li>
      ),
      style: {},
    },
    {
      ...
      key: (item, i, idOrder, style) => (
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <li style={style}>{item?.orderStatus} </li>
          {item?.userRules === "admin" ||
          item?.userRules === "Head Of Sales" ? (
            <Button
              disabled={
              ...
              style={{
                backgroundColor:
                  item?.paymentStatus !== "settlement" ? "red" : "#998970",
    ...
    },
    {
      ...
      key: (item) => (
        <div>
          {typeof item?.shippingDate === "number" ? (
            <TimestampToDate timestamp={item?.shippingDate} />
          ) : (
            formatDate(item?.shippingDate?.toDate?.())
          )}
          <Button
            disabled={item?.orderStatus === "sent" || item?.isDownloaded}
            onClick={() => setEditShipDateDialog({ open: true, id: item?.id })}
            style={{ padding: "0px", width: "20px", margin: "0px 0px 0px 2px" }}
            size="sm"
          >
    ...
    {
      label: "Shipping Cost",
      key: (item) => currency(item?.shippingCost),
      style: {},
    },
    {
      label: "Resi",
      key: (item, i, idOrder, style, edit) =>
        item.resi && edit !== i ? (
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <button
              disabled={item?.isResiSentToWASender}
              ...
              onClick={() =>
                setSendWADialog({
                  ...
                  data: item,
                  type: "resi_to_sender",
                  message: "Kirim resi ke pengirim?",
                })
              }
              style={
                item?.isResiSentToWASender
                  ? {
                      padding: "0px",
                      width: "20px",
                  ...
                  : { padding: "0px", width: "20px", margin: "0px 2px 0px 0px" }
              }
              size="sm"
            >
              <Whatsapp
                size={12}
                color={item?.isResiSentToWASender ? "gray" : "green"}
              />
            </button>
            {<TruncatedText text={item?.resi} maxLength={20} />}{" "}
            <Button
              onClick={() => setEdit(i)}
              style={{
                padding: "0px",
    ...
    },
    {
      ...
      key: (item) =>
        item.resiUpdate ? (
          typeof item?.resiUpdate === "number" ? (
            <TimestampToDate timestamp={item?.resiUpdate} />
          ) : (
            formatDate(item?.resiUpdate?.toDate())
          )
        ) : (
          ""
        ),
    ...
    { label: "Resi Crated By", key: (item) => item.resiCreatedBy, style: {} },
    {
      ...
      key: (item, i) => (
        <button
          style={
            item.isDownloaded
              ? { backgroundColor: "lightgray", padding: "5px" }
              : item?.paymentStatus === "settlement"
              ? { padding: "5px" }
              : { backgroundColor: "red", padding: "5px" }
          }
          disabled={item?.isDownloaded || item?.paymentStatus !== "settlement"}
          onClick={() => {
            setLoading(true);
            setModalDownload({
              userId: currentUser?.uid,
  ...
  useEffect(() => {
    const unsub = onSnapshot(columnRef, (doc) => {
      const source = doc.metadata.hasPendingWrites ? "Local" : "Server";
      // console.log(source, " data: ", doc.data());
      const selectedData = column?.filter?.((item) =>
        doc.data()?.columnOrder?.includes(item.label)
      );
        ...
        <Col md={3}>
          <Card className="shadow-sm">
            <Card.Body>
              ...
              <Card.Text>
                <h3 style={{ margin: "0px" }}>
                  {loadingOrder ? <Loading /> : totalOrdersCount}
                </h3>
        ...
        <Col md={3}>
          <Card className="shadow-sm">
            <Card.Body>
              ...
              <Card.Text>
                <h3 style={{ margin: "0px" }}>
                  {loadingOrder ? <Loading /> : totalOrdersPaidCount}
                </h3>
        ...
        <Col md={3}>
          <Card className="shadow-sm">
            <Card.Body>
              ...
              <Card.Text>
                <h3 style={{ margin: "0px" }}>
                  {loadingOrder ? <Loading /> : currency(revenue ?? totalOmset)}
                </h3>
        ...
        >
          ...
          <div className="section-table" id="table-section">
            <div className="table-container" ref={tableContainerRef}>
              <Table
                style={{ fontSize: "10px", position: "sticky", bottom: "20px" }}
                striped
                bordered
                hover
              >
                  ...
                  <tr style={{ whiteSpace: "nowrap" }}>
                    <th>
                      <input
                        ...
                        type="checkbox"
                        checked={selectedRows.length === mapData.length}
