...
const OrderList = () => {
    ...
    const [lengthPaid, setLengthPaid] = useState(0)
    const [lengthUnPaid, setLengthUnPaid] = useState(0)
    const paidLength = allOrders.filter(ord => ord?.paymentStatus === 'settlement').length
    useEffect(() => {
        const fetchData = async () => {
            let filterDate = []
            if (dateTimestamp.end) {
                filterDate.push(where("createdAt", ">=", dateTimestamp.start), where("createdAt", "<=", dateTimestamp.end))
            }
            const getDoc = query(collection(firestore, "orders"), ...filterDate, ...allFilters,
                // where('totalHargaProduk', '!=', '')
            );
            const snapshot = await getCountFromServer(getDoc);
            const items = snapshot.data().count;
            // console.log(items);
            setLengthAll(items)

            // paid
            const getDocPaid = query(collection(firestore, "orders"), ...filterDate, ...allFilters, where('paymentStatus', '==', 'settlement'));
            const snapshotPaid = await getCountFromServer(getDocPaid);
            const paid = snapshotPaid.data().count;
            setLengthPaid(paid);
            // console.log(paid);

            // unpaid
            const getDocUnPaid = query(collection(firestore, "orders"), ...filterDate, ...allFilters, where('paymentStatus', '!=', 'settlement'));
            const snapshotUnPaid = await getCountFromServer(getDocUnPaid);
            const Unpaid = snapshotUnPaid.data().count;
            setLengthUnPaid(Unpaid);

            // omset
            const sumOmset = httpsCallable(functions, 'sumOrdersPrice');
            const result = await sumOmset();
            console.log(result)
        };
        fetchData();
    }, [allFilters, dateTimestamp.end]);
    ...
    const showNext = ({ item }) => {
        if (list.length === 0) {
            ...
            const getDoc = query(collection(firestore, "orders"), ...allFilters, ...filter, orderBy("createdAt", "desc"), startAfter(item.createdAt), limit(length));
            const unsubscribe = onSnapshot(getDoc, (snapshot) => {
                const updatedData = snapshot.docs.map((doc) => ({
                    id: doc.id,
                    ...doc.data(),
    ...
    const filterByDate = async (start, end) => {
        try {
            ...
            // orders
            const refOrd = query(collection(firestore, "orders"), ...allFilters, where("createdAt", ">=", startTimestamp), where("createdAt", "<=", endTimestamp), orderBy('createdAt', 'desc'));
    ...
    // header 
    console.log(findDataUser?.rules === 'admin' || findDataUser?.rules === 'Head Of Sales')
    ...
    return (
        <div className="container">
                ...
                <Col md={3}>
                    <Card className="shadow-sm">
                        <Card.Body>
                            ...
                            <Card.Text>
                                <h3 style={{ margin: '0px' }}>{lengthPaid}</h3>
                ...
                <Col md={3}>
                    <Card className="shadow-sm">
                        <Card.Body>
                            ...
                            <Card.Text>
                                <h3 style={{ margin: '0px' }}>{lengthUnPaid}</h3>
            ...
            <ButtonGroup style={{ textAlign: 'center', float: 'right' }}>
                ...
                <Button disabled={list.length < length} style={{ whiteSpace: 'nowrap', backgroundColor: '#3D5E54', border: 'none' }} onClick={() => showNext({ item: mapData[mapData.length - 1] })}>{'Next->'}</Button>
            </ButtonGroup>
