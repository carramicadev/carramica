...
const TransactionCart = ({ allOrders }) => {
    // all order
    const filterDate = allOrders?.filter((all) => all.createdAt && all.createdAt !== undefined)
    ...
    const groupedDates = allDate?.reduce((acc, date) => {
        if (date) { // Ignore null values
            acc[date] = (acc[date] || 0) + 1;
        }
        return acc;
    }, {});
    const convertDate = Object.entries(groupedDates).reduce((acc, [date, count]) => {
        acc[date] = { count, date };
        return acc;
    }, {});
    ...
    // paid
    const filterDatePaid = allOrders?.filter((all) => all.createdAt && all.paymentStatus === 'settlement')
    ...
    const convertDatePaid = Object.entries(groupedDatesPaid).reduce((acc, [date, count]) => {
        acc[date] = { count, date };
        return acc;
    }, {});
    ...
    return <Line height={80} data={data} options={options} />;
};
