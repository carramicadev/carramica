...
export default function DialogAddUsers({ show, handleClose, setUpdate }) {
  ...
  const handleAdd = async (e) => {
    ...
    if (
      Object.values(findErros).some((err) => err !== "") &&
      show?.mode !== "edit"
    ) {
      ...
      try {
        if (show?.mode === "edit") {
          await setDoc(
            doc(firestore, "users", show?.data?.id),
            {
          ...
          enqueueSnackbar(`sukses mengubah user ${formData?.firstName}`, {
            variant: "success",
          });
        } else {
          ...
          await setDoc(
            doc(firestore, "users", result.data.uid),
            {
              email: formData.email,
              firstName: formData.firstName,
              lastName: formData.lastName,
          ...
          enqueueSnackbar(`sukses menambahkan user ${formData?.firstName}`, {
            variant: "success",
          });
        }
        // console.log(result)
        ...
        enqueueSnackbar(`gagal menambahkan user, ${e?.message}`, {
          variant: "error",
        });
  ...
  useEffect(() => {
    if (show?.mode === "edit") {
      setFormData(show?.data);
    } else {
      setFormData({ phone: "62" });
    }
  }, [show?.mode]);
  //   console.log(show);
  return (
    ...
    >
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {show?.mode === "edit" ? "Edit Sales/Users" : "Add Sales/Users"}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body
          style={{
          ...
          <Form>
            ...
            <Form.Group as={Row} controlId="formPhone">
              <Col sm={12}>
                ...
                <Form.Control
                  disabled={show?.mode === "edit" ? true : false}
            ...
            <Form.Group as={Row} controlId="formEmail">
              <Col sm={12}>
                ...
                <Form.Select
                  isInvalid={formError.rules ? true : false}
