...
import { collection, deleteDoc, doc, endBefore, getDocs, limit, limitToLast, orderBy, query, startAfter, Timestamp, where } from 'firebase/firestore';
import React, { useEffect, useState } from 'react';
import { Button, ButtonGroup, Card, Col, Row } from 'react-bootstrap';
import { BoxFill, CloudArrowDown, GraphUp, PencilSquare, PeopleFill, PersonSquare, TrashFill, XCircleFill } from 'react-bootstrap-icons';
...
const Contact = () => {
  ...
  useEffect(() => {
    if (page === 1) {
      const fetchData = async () => {
        const getDoc = query(collection(firestore, "contact"), orderBy("createdAt", "desc"));
        const documentSnapshots = await getDocs(getDoc);
        var items = [];
  ...
  useEffect(() => {
    if (page === 1) {
      const fetchData = async () => {
        const getDoc = query(collection(firestore, "contact"), orderBy("createdAt", "desc"), limit(20));
        const documentSnapshots = await getDocs(getDoc);
        var items = [];
  ...
  const showNext = ({ item }) => {
    if (allContacts.length === 0) {
      ...
      const fetchNextData = async () => {
        const getDoc = query(collection(firestore, "contact"), orderBy("createdAt", "desc"), startAfter(item.createdAt), limit(20));
        const documentSnapshots = await getDocs(getDoc);
        var items = [];
  ...
  const showPrevious = ({ item }) => {
    const fetchPreviousData = async () => {
      const getDoc = query(collection(firestore, "contact"), orderBy("createdAt", "desc"), endBefore(item.createdAt), limitToLast(20));
      const documentSnapshots = await getDocs(getDoc);
      var items = [];
  ...
  const filterByDate = async (start, end) => {
    try {
      ...
      })); const ref = query(collection(firestore, "contact"), where("createdAt", ">=", startTimestamp), where("createdAt", "<=", endTimestamp), limit(20));
      ...
      // orders
      const refOrd = query(collection(firestore, "orders"), where("createdAt", ">=", startTimestamp), where("createdAt", "<=", endTimestamp));
  ...
  // finc arpc/arpu
  const paidOrder = allOrders.filter((all) => all?.paymentStatus === 'settlement');
    ...
    <div className="container">
        ...
        <Col md={3}>
          <Card className="shadow-sm">
            <Card.Body>
              ...
              <Card.Text>
                <h3 style={{ margin: '0px' }}>{lengthContact}</h3>
        ...
        <Col md={3}>
          <Card className="shadow-sm">
            <Card.Body>
              ...
              <Card.Text>
                <h3 style={{ margin: '0px' }}>{jumlahPembeliAll}</h3>
        ...
        <Col md={3}>
          <Card className="shadow-sm">
            <Card.Body>
              ...
              <Card.Text>
                <h3 style={{ margin: '0px' }}>{parseFloat((allOrders.length / jumlahPembeliAll).toFixed(2))}</h3>
        ...
        <Col md={3}>
          <Card className="shadow-sm">
            <Card.Body>
              ...
              <Card.Text>
                <h3 style={{ margin: '0px' }}>{decimal(ARPC)}</h3>
      ...
      <div className="table-responsive">
          ...
          <tbody>
            {
              allContacts?.map((cont) => {
                const arrayInv = groupedData?.[cont?.phone]?.map((data) => data?.totalHargaProduk + data?.totalOngkir);
                // console.log(arrayInv)
                const lfitemeValue = arrayInv?.reduce((val, nilaiSekarang) => {
                  return val + nilaiSekarang
                }, 0)
                return <tr style={{ whiteSpace: 'nowrap' }} key={cont.id}>
                  <td>
                    <div className="d-flex align-items-center">
                      <span className="me-2">
                        <PersonSquare color={cont?.type === 'sender' ? '#3D5E54' : '#998970'} />
                  ...
                  <td>{formatDate(groupedData?.[cont?.phone]?.[groupedData?.[cont?.phone]?.length - 1]?.createdAt.toDate())}</td>
                  <td>
                    <button onClick={() => setDialogAdd({ open: true, data: {}, mode: 'edit', item: cont })} style={{ backgroundColor: '#998970' }} className="button button-primary"><PencilSquare /></button>
                    <button style={{ backgroundColor: 'red' }} className="button button-primary" onClick={() => handleDeleteClick(cont?.id)}>
                      <TrashFill />
                    </button>
                  </td>
                </tr>
        ...
        <ButtonGroup style={{ textAlign: 'center', float: 'right' }}>
          ...
          <Button disabled={page === 1} style={{ marginRight: '10px', whiteSpace: 'nowrap', backgroundColor: '#3D5E54', border: 'none' }} onClick={() => showPrevious({ item: allContacts[0] })}>{'<-Prev'}</Button>
          ...
          <Button disabled={allContacts.length < 20} style={{ whiteSpace: 'nowrap', backgroundColor: '#3D5E54', border: 'none' }} onClick={() => showNext({ item: allContacts[allContacts.length - 1] })}>{'Next->'}</Button>
        </ButtonGroup>
      </div>
      <DialogAddContact
        show={dialogAdd}
